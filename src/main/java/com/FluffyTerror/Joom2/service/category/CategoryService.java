...
public class CategoryService implements ICategoryService {
    ...
    public Category getCategoryById(Long id) {
        return categoryRepository.findById(id).orElseThrow(()->new ResourceNotFoundException("No such category in the list"));
    }
    ...
    public Category updateCategory(Category category, Long id) {
        return Optional.ofNullable(getCategoryById(id)).map(oldCategory->{oldCategory.setName(category.getName());
                ...
                .orElseThrow(()-> new ResourceNotFoundException("No such category in the list"));
    }

    @Override
    public void deleteCategoryById(Long id) {
        categoryRepository.findById(id).ifPresentOrElse(categoryRepository::delete,
                ()->{throw new ResourceNotFoundException("No such category in the list");});
